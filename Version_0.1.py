...
class StartPage:
    ...
    def Initialization(self):
        db = pymssql.connect('PC-202204291329', 'yuyue', 'mayuchen001129mm', 'EMIS')  # 服务器名,账户,密码,数据库名
        cursor = db.cursor()
        print('建立数据库中......')
        sql = """
		-- 创建EMIS数据库
		...
		(
			...
			FILENAME='C:\\Program Files\\Microsoft SQL Server\\MSSQL16.MSSQLSERVER\\MSSQL\\DATA\\EMIS.mdf', --主文件文件名
		...
		(
			...
			filename='C:\\Program Files\\Microsoft SQL Server\\MSSQL16.MSSQLSERVER\\MSSQL\\DATA\\EMIS.mdf\\EMIS_log.ldf',--日志文件屋里文件名
			SIZE=5MB,--日志文件初始大小
			filegrowth=0 --启动自动增长
		)
		"""
    ...
    def jianbiao(self):
		...
		number1 varchar(0) DEFAULT NULL,
		dizhi varchar(0) DEFAULT NULL,
		...
		---向卖家信息数据表中插入数据
		INSERT INTO t_buyer VALUES ('112233', '测试', '测试', '测试');
		...
		---向订单信息表中插入数据
		INSERT INTO t_order VALUES ('123', '456', '789', '测试', '测试');
...
class All_admin:
    def __init__(self, parent_window):
        ...
        Button(self.window, text="充电桩编号型号管理", font=tkFont.Font(size=16), width=32, height=2,
               command=lambda: AdminManage(self.window),
               fg='white', bg='gray', activebackground='black', activeforeground='white').pack()
        Button(self.window, text="充电桩所用电气型号", font=tkFont.Font(size=16), width=32, height=2,
               command=lambda: User_AdminManage(self.window),
               fg='white', bg='gray', activebackground='black', activeforeground='white').pack()
        Button(self.window, text="质保时间信息管理", font=tkFont.Font(size=16), width=32, height=2,
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        self.frame_left_top = tk.Frame(width=300, height=280)  # 指定框架，在窗口上可以显示，这里指定四个框架
        self.frame_right_top = tk.Frame(width=300, height=250)
        self.frame_center = tk.Frame(width=650, height=250)
        self.frame_bottom = tk.Frame(width=750, height=70)

        # 定义下方中心列表区域
        self.columns = ("型号名称", "编号", "发货时间", "发货地址", "启用地址", "启用时间", "质保一年", "质保两年")
        self.tree = ttk.Treeview(self.frame_center, show="headings", height=23, columns=self.columns)
        # 添加竖直滚动条
        ...
        # 表格的标题
        self.tree.column("型号名称", width=55, anchor='center')
        self.tree.column("编号", width=80, anchor='center')
        self.tree.column("发货时间", width=80, anchor='center')
        self.tree.column("发货地址", width=55, anchor='center')
        self.tree.column("启用地址", width=50, anchor='center')
        self.tree.column("启用时间", width=50, anchor='center')
        self.tree.column("质保一年", width=80, anchor='center')
        self.tree.column("质保两年", width=80, anchor='center')
        ...
        # 打开数据库连接
        db = pymssql.connect('PC-202204291329', 'yuyue', 'mayuchen001129mm', 'EMIS')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        ...
        try:
            ...
            for row in results:
                ...
                self.number1.append(row[2])
                self.dizhi.append(row[3])
                self.gender.append(row[4])
                self.age.append(row[5])
                self.new2.append(row[6])
                self.new1.append(row[7])
        except:
            messagebox.showinfo('警告！', '数据库连接失败！')
        db.close()  # 关闭数据库连接

        print("test***********************")
        for i in range(min(len(self.id), len(self.name),len(self.number1),len(self.dizhi), len(self.gender), len(self.age), len(self.new2), len(self.new1))):  # 写入数据
            self.tree.insert('', i, values=(self.id[i], self.name[i], self.number1[i],self.dizhi[i], self.gender[i], self.age[i], self.new2[i], self.new1[i]))
        ...
        self.top_title.grid(row=1, column=1, columnspan=1, sticky=N+S+E+W, padx=5, pady=5)  # NSEW表示允许组件向4个方向都可以拉伸

        # 定义下方区域
        self.chaxun = StringVar()
        self.right_bottom_gender_entry = Entry(self.frame_bottom, textvariable=self.chaxun, font=('Verdana', 15))
        self.right_bottom_button = ttk.Button(self.frame_bottom, text='内容查询', width=30, command=self.put_data)
        self.right_bottom_button.grid(row=0, column=0, padx=20, pady=20)  # 位置设置
        self.right_bottom_gender_entry.grid(row=0, column=1)

        self.left_top_frame = tk.Frame(self.frame_left_top, bd=10)
        self.var_id = StringVar()  # 声明学号

        self.var_name = StringVar()  # 声明姓名
        self.var_number1 = StringVar()
        self.var_dizhi = StringVar()
        self.var_gender = StringVar()  # 声明性别
        self.var_age = StringVar()  # 声明年龄
        self.var_new2 = StringVar()
        self.var_new1 = StringVar()
        # 商品id
        self.right_top_id_label = Label(self.frame_left_top, text="型号名称：", font=('Verdana', 15))
        ...
        # 商品名称
        self.right_top_name_label = Label(self.frame_left_top, text="编号：", font=('Verdana', 15))
        ...
        self.right_top_number1_label = Label(self.frame_left_top, text="发货时间：", font=('Verdana', 15))
        self.right_top_number1_entry = Entry(self.frame_left_top, textvariable=self.var_number1, font=('Verdana', 15))
        self.right_top_number1_label.grid(row=3, column=0)  # 位置设置
        self.right_top_number1_entry.grid(row=3, column=1)

        self.right_top_dizhi_label = Label(self.frame_left_top, text="发货地址：", font=('Verdana', 15))
        self.right_top_dizhi_entry = Entry(self.frame_left_top, textvariable=self.var_dizhi, font=('Verdana', 15))
        self.right_top_dizhi_label.grid(row=4, column=0)  # 位置设置
        self.right_top_dizhi_entry.grid(row=4, column=1)


        # 商品价格
        self.right_top_gender_label = Label(self.frame_left_top, text="启用地址：", font=('Verdana', 15))
        ...
        # 销售数量
        self.right_top_age_label = Label(self.frame_left_top, text="启用时间：", font=('Verdana', 15))
        self.right_top_age_entry = Entry(self.frame_left_top, textvariable=self.var_age, font=('Verdana', 15))
        self.right_top_age_label.grid(row=6, column=0)  # 位置设置
        self.right_top_age_entry.grid(row=6, column=1)

        self.right_top_new2_label = Label(self.frame_left_top, text="质保一年：", font=('Verdana', 15))
        self.right_top_new2_entry = Entry(self.frame_left_top, textvariable=self.var_new2, font=('Verdana', 15))
        self.right_top_new2_label.grid(row=7, column=0)  # 位置设置
        self.right_top_new2_entry.grid(row=7, column=1)

        self.right_top_new1_label = Label(self.frame_left_top, text="质保两年：", font=('Verdana', 15))
        self.right_top_new1_entry = Entry(self.frame_left_top, textvariable=self.var_new1, font=('Verdana', 15))
        self.right_top_new1_label.grid(row=8, column=0)  # 位置设置
        self.right_top_new1_entry.grid(row=8, column=1)
        ...
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='新建充电桩信息', width=20, command=self.new_row)
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中充电桩信息', width=20, command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='删除选中充电桩信息', width=20, command=self.del_row)
        self.right_top_button1.grid(row=10, column=10, padx=10, pady=10)
        # 定义下方区域，查询功能块
        self.chaxun = StringVar()
        self.right_bottom_gender_entry = Entry(self.frame_bottom, textvariable=self.chaxun, font=('Verdana', 15))
        self.right_bottom_button = ttk.Button(self.frame_bottom, text='内容查询', width=10, command=self.put_data)
        self.right_bottom_button.grid(row=20, column=20, padx=20, pady=20)  # 位置设置
        self.right_bottom_gender_entry.grid(row=0, column=1)
    ...
    def click(self, event):
        ...
        self.var_number1.set(self.row_info[2])
        self.var_dizhi.set(self.row_info[3])
        self.var_gender.set(self.row_info[4])
        self.var_age.set(self.row_info[5])
        self.var_new2.set(self.row_info[6])
        self.var_new1.set(self.row_info[7])
    ...
    def new_row(self):
        ...
        if str(self.var_id.get()) in self.id:
            messagebox.showinfo('警告！', '该充电桩已存在！')
        else:
            if self.var_id.get() != ''  and self.var_name.get() != '' and self.var_number1.get() != ''and self.var_dizhi.get() != ''and self.var_gender.get() != '' and self.var_age.get() != '':
                # 打开数据库连接
                db = pymssql.connect('PC-202204291329', 'yuyue', 'mayuchen001129mm', 'EMIS')
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO t_goods(id,  name,number1, dizhi,  gender, age, new2 ,new1) \
				       VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % \
                      (self.var_id.get(), self.var_name.get(), self.var_number1.get(), self.var_dizhi.get(), self.var_gender.get(), self.var_age.get(),
                       self.var_new2.get(), self.var_new1.get())  # SQL 插入语句
                try:
                ...
                self.number1.append(self.var_number1.get())
                self.dizhi.append(self.var_dizhi.get())
                self.gender.append(self.var_gender.get())
                self.age.append(self.var_age.get())
                self.new2.append(self.var_new2.get())
                self.new1.append(self.var_new1.get())
                self.tree.insert('', len(self.id) - 1, values=(
                    self.id[len(self.id) - 1], self.name[len(self.id) - 1], self.number1[len(self.id) - 1], self.dizhi[len(self.id) - 1],self.gender[len(self.id) - 1],
                    self.age[len(self.id) - 1], self.new2[len(self.id) - 1], self.new1[len(self.id) - 1]))
                self.tree.update()
                messagebox.showinfo('提示！', '插入成功！')
            else:
                messagebox.showinfo('警告！', '请填写信息')
    ...
    def updata_row(self):
        ...
        if res == True:
            ...
            db = pymssql.connect('PC-202204291329', 'yuyue', 'mayuchen001129mm', 'EMIS')
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "UPDATE t_goods SET id = '%s', name = '%s', number1 = '%s', dizhi = '%s', gender = '%s', age = '%s', new2 = '%s' , new1 = '%s' where id = '%s'" % (
            self.var_id.get(),  self.var_name.get(),self.var_number1.get(),self.var_dizhi.get(), self.var_gender.get(), self.var_age.get(),self.var_new2.get()
            ,self.var_new1.get(), self.id1)  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
            ...
            self.number1[id_index] = self.var_number1.get()
            self.dizhi[id_index] = self.var_dizhi.get()
            self.gender[id_index] = self.var_gender.get()
            self.age[id_index] = self.var_age.get()
            self.new2[id_index] = self.var_new2.get()
            self.new1[id_index] = self.var_new1.get()

            self.tree.item(self.tree.selection()[0], values=(
                self.var_id.get(), self.var_name.get(),self.var_number1.get(),self.var_dizhi.get(), self.var_gender.get(),
                self.var_age.get(), self.var_new2.get(), self.var_new1.get()))  # 修改对于行信息
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            del self.number1[id_index]
            del self.dizhi[id_index]
            del self.gender[id_index]
            del self.age[id_index]
            del self.new2[id_index]
            del self.new1[id_index]
            print(self.id)
...
class User_AdminManage:
    def __init__(self, parent_window):
        ...
        # 定义下方中心列表区域
        self.columns = ("序号", "编号", "厂家", "电气元器件", "型号")
        ...
        # 表格的标题
        self.tree.column("序号", width=50, anchor='center')  # 表示列,不显示
        self.tree.column("编号", width=100, anchor='center')
        self.tree.column("厂家", width=80, anchor='center')
        self.tree.column("电气元器件", width=80, anchor='center')
        ...
        print("test***********************")
        for i in range(min(len(self.id), len(self.name), len(self.gender), len(self.age), len(self.dizhi1))):  # 写入数据
            self.tree.insert('', i, values=(self.id[i], self.name[i], self.gender[i], self.age[i], self.dizhi1[i]))
        ...
        self.right_top_name_entry.grid(row=2, column=1)
        # 性别
        self.right_top_gender_label = Label(self.frame_left_top, text="厂家：", font=('Verdana', 15))
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_gender,font=('Verdana', 15))
        self.right_top_gender_label.grid(row=3, column=0)  # 位置设置
        self.right_top_gender_entry.grid(row=3, column=1)
        # 年龄
        self.right_top_age_label = Label(self.frame_left_top, text="电气元器件：", font=('Verdana', 15))
        self.right_top_age_entry = Entry(self.frame_left_top, textvariable=self.var_age,font=('Verdana', 15))
        self.right_top_age_label.grid(row=4, column=0)  # 位置设置
        self.right_top_age_entry.grid(row=4, column=1)

        self.right_top_dizhi1_label = Label(self.frame_left_top, text="型号：", font=('Verdana', 15))
        self.right_top_dizhi1_entry = Entry(self.frame_left_top, textvariable=self.var_dizhi1,font=('Verdana', 15))
        self.right_top_dizhi1_label.grid(row=5, column=0)  # 位置设置
        self.right_top_dizhi1_entry.grid(row=5, column=1)

        # 定义右上方区域
        self.right_top_title = Label(self.frame_right_top)
        ...
        self.chaxun = StringVar()
        self.right_bottom_gender_entry = Entry(self.frame_bottom, textvariable=self.chaxun, font=('Verdana', 15))
        self.right_bottom_button = ttk.Button(self.frame_bottom, text='内容查询', width=20, command=self.put_data)
        self.right_bottom_button.grid(row=0, column=0, padx=20, pady=20)  # 位置设置
        self.right_bottom_gender_entry.grid(row=0, column=1)
    ...
    def click(self, event):
        ...
        self.var_dizhi1.set(self.row_info[4])
        self.right_top_id_entry = Entry(self.frame_left_top, state='disabled', textvariable=self.var_id,
                                        font=('Verdana', 15))
    ...
    def new_row(self):
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() != '' and self.var_age.get() != '':
                ...
                db = pymssql.connect('PC-202204291329', 'yuyue', 'mayuchen001129mm', 'EMIS')
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO t_buyer(id, name, gender, age, dizhi1) \
				       VALUES ('%s', '%s', '%s', '%s', '%s')" % \
                      (self.var_id.get(), self.var_name.get(), self.var_gender.get(), self.var_age.get(), self.var_dizhi1.get())  # SQL 插入语句
                try:
                    cursor.execute(sql)  # 执行sql语句
                    db.commit()  # 提交到数据库执行
                except:
                ...
                self.gender.append(self.var_gender.get())
                self.age.append(self.var_age.get())
                self.dizhi1.append((self.var_dizhi1.get()))
                self.tree.insert('', len(self.id) - 1, values=(
                self.id[len(self.id) - 1], self.name[len(self.id) - 1], self.gender[len(self.id) - 1],
                self.age[len(self.id) - 1], self.dizhi1[len(self.id) - 1]))
                self.tree.update()
                messagebox.showinfo('提示！', '插入成功！')
            else:
    ...
    def updata_row(self):
        ...
        if res == True:
            ...
            db = pymssql.connect('PC-202204291329', 'yuyue', 'mayuchen001129mm', 'EMIS')
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "UPDATE t_buyer SET id = '%s', name = '%s', gender = '%s', age = '%s', dizhi1 = '%s' \
				where id = '%s'" % (
            self.var_id.get(), self.var_name.get(), self.var_gender.get(), self.var_age.get(), self.var_dizhi1.get() ,self.id1)  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
                messagebox.showinfo('提示！', '更新成功！')
            ...
            self.gender[id_index] = self.var_gender.get()
            self.age[id_index] = self.var_age.get()
            self.dizhi1[id_index] = self.var_dizhi1.get()

            self.tree.item(self.tree.selection()[0], values=(
                self.var_id.get(), self.var_name.get(), self.var_gender.get(),
                self.var_age.get(), self.var_dizhi1.get()))  # 修改对于行信息
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            db = pymssql.connect('PC-202204291329', 'yuyue', 'mayuchen001129mm', 'EMIS')
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "DELETE FROM t_buyer WHERE id '%s'" % (self.row_info[0])  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
                messagebox.showinfo('提示！', '删除成功！')
...
class Buy_AdminManage:
    def __init__(self, parent_window):
        ...
        # 定义下方中心列表区域
        self.columns = ("序号", "厂家", "电气元器件", "型号", "质保时间")
        self.tree = ttk.Treeview(self.frame_center, show="headings", height=18, columns=self.columns)
        self.vbar = ttk.Scrollbar(self.frame_center, orient=VERTICAL, command=self.tree.yview)
        # 定义树形结构与滚动条
        self.tree.configure(yscrollcommand=self.vbar.set)
        ...
        # 表格的标题
        self.tree.column("序号", width=100, anchor='center')  # 表示列,不显示
        self.tree.column("厂家", width=100, anchor='center')
        self.tree.column("电气元器件", width=100, anchor='center')
        self.tree.column("型号", width=100, anchor='center')
        self.tree.column("质保时间", width=100, anchor='center')
        ...
        # 定义顶部区域
        # 定义左上方区域
        self.top_title = Label(self.frame_left_top, text="内容信息:", font=('Verdana', 20))
        self.top_title.grid(row=0, column=0, columnspan=2, sticky=NSEW, padx=50, pady=10)
        ...
        self.right_top_name_label = Label(self.frame_left_top, text="厂家：", font=('Verdana', 15))
        self.right_top_name_entry = Entry(self.frame_left_top, textvariable=self.var_name, font=('Verdana', 15))
        self.right_top_name_label.grid(row=2, column=0)  # 位置设置
        self.right_top_name_entry.grid(row=2, column=1)

        self.right_top_gender_label = Label(self.frame_left_top, text="电气元器件：", font=('Verdana', 15))
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_gender, font=('Verdana', 15))
        self.right_top_gender_label.grid(row=3, column=0)  # 位置设置
        self.right_top_gender_entry.grid(row=3, column=1)
        ...
        self.right_top_gender_label = Label(self.frame_left_top, text="质保时间：", font=('Verdana', 15))
        ...
        self.tree.bind('<Button-1>', self.click)  # 左键获取位置
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='新建质保信息', width=20, command=self.new_row)
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中内容信息', width=20, command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='删除选中内容信息', width=20, command=self.del_row)
    ...
    def new_row(self):
        ...
        if str(self.var_id.get()) in self.SNO:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() != '' and self.var_age.get() != '' and self.var_dep.get() != '':
                ...
                messagebox.showinfo('警告！', '请填写内容信息')
...
class AboutPage:
    def __init__(self, parent_window):
        ...
        label = tk.Label(self.window, text='内容信息管理系统', bg='green', font=('Verdana', 20), width=30, height=2)
        label.pack()

        Label(self.window, text='作者：愉悦/马宇辰', font=('Verdana', 18)).pack(pady=30)
        Label(self.window, text='联系：18913391836', font=('Verdana', 18)).pack(pady=5)
